...
export interface ProjectSummary {
  ...
  cameraSchedule: CameraSummaryItem[];
}
...
const DEFAULT_RECORDING_DAYS = 30;
const DEFAULT_FRAMES_PER_SECOND = 15;
...
function findCameraProduct(
  ...
  const cameraName =
    normaliseText(camera.name);

  return (
    cameraDatabase.find((product) => {
      ...
      const model =
        normaliseText(product.model);

      return (
        cameraName === fullProductName ||
        cameraName === model ||
        cameraName.includes(model)
      );
    }) ?? null
  );
}
...
function recommendPoEPowerBudget(
  ...
  if (totalCameraPower <= 0) {
    return 0;
  }

  const powerWithHeadroom =
    totalCameraPower * 1.25;
  ...
  return (
    Math.ceil(powerWithHeadroom / 100) *
    100
  );
}
...
function estimateStorageTB(
  ...
  const totalBitrateMbps =
    cameras.reduce(
      (total, camera) => {
        const product =
          findCameraProduct(camera);

        const resolution =
          product?.resolutionMP ?? 4;
...
function buildCameraSchedule(
  ...
  const schedule =
    new Map<string, CameraSummaryItem>();
...
export function calculateProjectSummary({
  ...
  return {
    ...
    averageResolutionMP:
      Number(
        averageResolutionMP.toFixed(1),
      ),

    averageIRRange:
      Number(
        averageIRRange.toFixed(1),
      ),
